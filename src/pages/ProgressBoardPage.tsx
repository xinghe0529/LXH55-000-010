...
export default function ProgressBoardPage() {
  ...
  const [households, setHouseholds] = useState<Array<{ id: string; unit: string; floor: number; roomNumber: string }>>([]);
  ...
  const [issues, setIssues] = useState<Array<IssueFeedback & {
  ...
  const [issueForm, setIssueForm] = useState({
    category: 'noise' as IssueFeedbackCategory,
    title: '',
    description: '',
    priority: 'medium' as IssueFeedbackPriority,
    householdId: '',
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const [p, prog, fin, vr, reports, pay, issueList, issueStat, hhList] = await Promise.all([
          api.getProposal(id),
          api.getProgress(id),
          api.getFinances(id),
          ...
          api.getProposalIssues(id).catch(() => []),
          api.getIssueStats(id).catch(() => ({ total: 0, pending: 0, processing: 0, resolved: 0, closed: 0 })),
          api.getHouseholds(id).catch(() => []),
        ]);
        if (!alive) return;
        setProposal(p);
        setProgress(prog);
        ...
        setIssues(issueList);
        setIssueStats(issueStat);
        setHouseholds(hhList);
        if (hhList.length > 0) {
          setIssueForm((prev) => ({ ...prev, householdId: hhList[0].id }));
        }
      } catch (e) {
  ...
  const handleSubmitIssue = async () => {
    if (!id || !issueForm.householdId || !issueForm.title || !issueForm.description) {
      showToast('请填写完整的问题信息', 'error');
      return;
    }
    setSubmittingIssue(true);
    try {
      await api.createIssueFeedback({
        proposalId: id,
        householdId: issueForm.householdId,
        category: issueForm.category,
        title: issueForm.title,
        description: issueForm.description,
        priority: issueForm.priority,
      });
      showToast('问题反馈提交成功', 'success');
      ...
      const [issueList, issueStat] = await Promise.all([
        api.getProposalIssues(id),
        api.getIssueStats(id),
      ]);
      setIssues(issueList);
      setIssueStats(issueStat);
    } catch (e) {
    ...
    <div className="animate-fade-in">
            ...
            <div className="card p-6 grain-overlay mb-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">施工问题与建议收集</p>
                  </div>
                </div>
                <Link
                  to="/issues"
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="p-3 rounded-xl bg-orange-50 border border-orange-100 text-center">
                  <div className="text-2xl font-display font-black text-orange-700">
                    {issueStats.total}
                  </div>
                  <div className="text-[11px] text-orange-600 font-medium">总反馈数</div>
                </div>
                <div className="p-3 rounded-xl bg-yellow-50 border border-yellow-100 text-center">
                  <div className="text-2xl font-display font-black text-yellow-700">
                    {issueStats.pending + issueStats.processing}
                  </div>
                  <div className="text-[11px] text-yellow-600 font-medium">待处理/处理中</div>
                </div>
              </div>
                  ...
                  {issues.slice(0, 3).map((issue) => (
                    ...
                    >
                      <div className="flex items-start gap-2">
                        ...
                        <span className={cn('badge !text-[10px] !px-1.5 !py-0.5 flex-shrink-0 mt-0.5', ISSUE_STATUS_COLORS[issue.status])}>
                          {ISSUE_STATUS_LABELS[issue.status]}
                        </span>
                      </div>
                      <div className="text-sm font-medium text-slate-700 mt-1 line-clamp-1">
                        {issue.title}
                      </div>
                      <div className="text-[11px] text-slate-500 mt-1">
                        {new Date(issue.createdAt).toLocaleDateString('zh-CN')}
                        {issue.replies.length > 0 && ` · ${issue.replies.length} 条回复`}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {issues.length === 0 && (
                <div className="text-center py-6 text-slate-400">
                  ...
                  <div className="text-sm">暂无问题反馈</div>
                </div>
              )}
      ...
      {showIssueModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setShowIssueModal(false)} />
          <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-xl animate-slide-up">
            <div className="px-6 py-5 border-b border-slate-100">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-display font-bold text-lg text-slate-900">提交问题反馈</h3>
                    <p className="text-xs text-slate-500">
                      {proposal?.communityName} {proposal?.buildingNumber}
                    </p>
                  </div>
                </div>
                <button onClick={() => setShowIssueModal(false)} className="p-2 rounded-lg hover:bg-slate-100 transition-colors">
            ...
            <div className="px-6 py-5 space-y-5">
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['noise', 'safety', 'quality', 'schedule', 'other'] as IssueFeedbackCategory[]).map((cat) => {
                    const active = issueForm.category === cat;
                    return (
                      <button
                        key={cat}
                        onClick={() => setIssueForm((prev) => ({ ...prev, category: cat }))}
                        className={cn(
                      ...
                      >
                        {cat === 'noise' && <Volume2 className={cn('w-5 h-5 mx-auto mb-1', active ? 'text-primary-600' : 'text-slate-400')} />}
                        {cat === 'safety' && <ShieldAlert className={cn('w-5 h-5 mx-auto mb-1', active ? 'text-primary-600' : 'text-slate-400')} />}
                        {cat === 'quality' && <Wrench className={cn('w-5 h-5 mx-auto mb-1', active ? 'text-primary-600' : 'text-slate-400')} />}
                        {cat === 'schedule' && <Calendar className={cn('w-5 h-5 mx-auto mb-1', active ? 'text-primary-600' : 'text-slate-400')} />}
                        {cat === 'other' && <MoreHorizontal className={cn('w-5 h-5 mx-auto mb-1', active ? 'text-primary-600' : 'text-slate-400')} />}
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {(['low', 'medium', 'high', 'urgent'] as IssueFeedbackPriority[]).map((p) => {
                    const active = issueForm.priority === p;
                    return (
                      <button
                        key={p}
                        onClick={() => setIssueForm((prev) => ({ ...prev, priority: p }))}
                        className={cn(
                      ...
                      >
                        {p === 'urgent' && <Zap className={cn('w-4 h-4 mx-auto mb-0.5', active ? 'text-red-600' : 'text-slate-400')} />}
                        <div className={cn('text-[11px] font-semibold', active ? 'text-slate-900' : 'text-slate-500')}>
              ...
              <div>
                <label className="label-base">问题标题</label>
                <input
                  type="text"
                  ...
                  maxLength={100}
                />
                <div className="text-xs text-slate-400 mt-1 text-right">{issueForm.title.length}/100</div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  maxLength={1000}
                />
                <div className="text-xs text-slate-400 mt-1 text-right">{issueForm.description.length}/1000</div>
              </div>

              <div>
                <label className="label-base">您的住户信息</label>
                <select
                  value={issueForm.householdId}
                  onChange={(e) => setIssueForm((prev) => ({ ...prev, householdId: e.target.value }))}
                  className="input-base"
                >
                  <option value="">请选择您的住户</option>
                  {households.map((h) => (
                    <option key={h.id} value={h.id}>
                      {h.unit}单元 {h.floor}楼 {h.roomNumber}室
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-end gap-3">
              <button onClick={() => setShowIssueModal(false)} className="btn-outline !px-5 !py-2.5">
                取消
              </button>
              <button
                onClick={handleSubmitIssue}
                disabled={submittingIssue || !issueForm.title.trim() || !issueForm.description.trim() || !issueForm.householdId}
